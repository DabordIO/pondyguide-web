...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: buildOpenGraph({ path: `/ta/history/lives-and-legacies/${slug}`, image: entry.en.photo ? `/figures/${entry.en.photo}` : undefined, locale: "ta_IN" }),
    alternates: {
      ...
      {en.photo && (
        <div style={{ display: "flex", gap: 24, margin: "24px 0 28px", alignItems: "flex-start" }}>
          <div>
            ...
            {ta.photoCaption && (
              <p style={{ fontSize: 11, color: "#a39990", fontStyle: "italic", marginTop: 6, maxWidth: 160, lineHeight: 1.4 }}>{ta.photoCaption}</p>
            )}
          </div>
        </div>
      )}
