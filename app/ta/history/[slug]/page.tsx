import { notFound } from "next/navigation";
import { buildOpenGraph } from "@/lib/metadata";
import type { Metadata } from "next";
import Image from "next/image";
import { historyArticles } from "@/data/history";
import { historyArticlesTa } from "@/data/ta/history";
import { historyArticlesFr } from "@/data/fr/history";
import ArticleBody from "@/components/ArticleBody";
import AppBanner from "@/components/AppBanner";
import JsonLd from "@/components/JsonLd";
import LanguageToggle from "@/components/LanguageToggle";
import Link from "next/link";

export async function generateStaticParams() {
  return historyArticlesTa.map(a => ({ slug: a.id }));
}

export const dynamicParams = false;

function findEntry(slug: string) {
  const ta = historyArticlesTa.find(a => a.id === slug);
  const en = historyArticles.find(a => a.id === slug);
  if (!ta || !en) return null;
  return { ta, en };
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params;
  const entry = findEntry(slug);
  if (!entry) return {};
  const { ta, en } = entry;
  const hasFr = historyArticlesFr.some(a => a.id === slug);
  return {
    title: ta.metaTitle ?? ta.title,
    description: ta.metaDescription ?? ta.teaser,
    openGraph: buildOpenGraph({ path: `/ta/history/${slug}`, image: en.photo ? `/${en.photoFolder ?? "history"}/${en.photo}` : undefined, locale: "ta_IN" }),
    alternates: {
      canonical: `/ta/history/${slug}`,
      languages: {
        en: `/history/${slug}`,
        ta: `/ta/history/${slug}`,
        ...(hasFr ? { fr: `/fr/history/${slug}` } : {}),
        "x-default": `/history/${slug}`,
      },
    },
  };
}

export default async function HistoryArticlePageTa({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const entry = findEntry(slug);
  if (!entry) notFound();
  const { ta, en } = entry;
  const hasFr = historyArticlesFr.some(a => a.id === slug);

  const idx = historyArticlesTa.findIndex(a => a.id === slug);
  const prev = idx > 0 ? historyArticlesTa[idx - 1] : null;
  const next = idx < historyArticlesTa.length - 1 ? historyArticlesTa[idx + 1] : null;

  return (
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "40px 24px 80px", position: "relative" }}>
      <LanguageToggle enHref={`/history/${slug}`} taHref={`/ta/history/${slug}`} frHref={hasFr ? `/fr/history/${slug}` : undefined} current="ta" />
      <JsonLd
        data={{
          "@context": "https://schema.org",
          "@type": "Article",
          headline: ta.title,
          description: ta.teaser,
          image: en.photo ? `https://www.pondyguide.com/${en.photoFolder ?? "history"}/${en.photo}` : undefined,
          url: `https://www.pondyguide.com/ta/history/${en.id}`,
          author: { "@type": "Organization", name: "Pondy Guide" },
          publisher: { "@type": "Organization", name: "Pondy Guide" },
        }}
      />
      <Link href="/ta/history" style={{ fontSize: 13, color: "#d4711a", textDecoration: "none", fontWeight: 600 }}>← வரலாறு</Link>

      {en.photo && (
        <div style={{ position: "relative", width: "100%", aspectRatio: "16/9", borderRadius: 14, overflow: "hidden", margin: "24px 0 28px" }}>
          <Image src={`/${en.photoFolder ?? "history"}/${en.photo}`} alt={ta.title} fill style={{ objectFit: "cover", objectPosition: "center" }} />
        </div>
      )}

      <h1 style={{ fontFamily: "var(--font-playfair), Georgia, serif", fontSize: "clamp(1.75rem, 5vw, 2.5rem)", fontWeight: 700, color: "#1c1917", margin: "20px 0 16px", lineHeight: 1.2 }}>
        {ta.title}
      </h1>

      {ta.teaser && (
        <p style={{ fontSize: "1.1rem", color: "#6b6560", lineHeight: 1.75, marginBottom: 32, paddingBottom: 32, borderBottom: "1px solid #e8ddd4", fontStyle: "italic" }}>
          {ta.teaser}
        </p>
      )}

      <ArticleBody text={ta.body} />

      <AppBanner />

      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 48, paddingTop: 24, borderTop: "1px solid #e8ddd4" }}>
        {prev ? <Link href={`/ta/history/${prev.id}`} style={{ fontSize: 14, color: "#d4711a", textDecoration: "none", fontWeight: 600 }}>← {prev.title}</Link> : <span />}
        {next ? <Link href={`/ta/history/${next.id}`} style={{ fontSize: 14, color: "#d4711a", textDecoration: "none", fontWeight: 600 }}>{next.title} →</Link> : <span />}
      </div>
    </div>
  );
}
