...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: buildOpenGraph({ path: `/fr/discover/landmarks/${slug}`, image: en.photo ? `/sites/${en.photo}` : undefined, locale: "fr_FR" }),
    alternates: {
      canonical: `/fr/discover/landmarks/${slug}`,
      languages: {
        en: `/discover/landmarks/${slug}`,
        fr: `/fr/discover/landmarks/${slug}`,
        ...(entry.hasTa ? { ta: `/ta/discover/landmarks/${slug}` } : {}),
    ...
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "40px 24px 80px", position: "relative" }}>
      ...
      {en.photo && (
        <div style={{ position: "relative", width: "100%", height: 320, borderRadius: 14, overflow: "hidden", margin: "24px 0 28px" }}>
          <Image src={`/sites/${en.photo}`} alt={fr.name} fill priority style={{ objectFit: "cover", objectPosition: en.photoPosition ?? "center" }} />
        </div>
      )}
