...
function GuideCard({ title, slug, photos, span }: { title: string; slug: string; photos: string[]; span: number }) {
  ...
  return (
    <Link href={`/fr/restaurants/guides/${slug}`} className="guide-card" style={{ gridColumn: `span ${span}`, textDecoration: "none", background: "#fff", border: "1px solid #e8ddd4", borderRadius: 14, overflow: "hidden", display: "flex", flexDirection: "column", height: "100%", minHeight: 180 }}>
...
export default function RestaurantsPageFr() {
  return (
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: "56px 24px 80px", position: "relative" }}>
      ...
      <Breadcrumbs items={[{ label: "Accueil", href: "/fr" }, { label: "Restaurants", href: "/fr/restaurants" }]} />
