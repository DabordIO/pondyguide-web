...
const READY_GUIDES_EN = hotelGuides.filter(g => g.intro && g.blurbs);

function findGuide(slug: string) {
  const en = READY_GUIDES_EN.find(g => g.slug === slug);
  if (!en) return null;
  const fr = hotelGuidesFr.find(g => g.id === en.id);
  if (!fr) return null;
  return { en, fr };
}
...
export async function generateStaticParams() {
  return READY_GUIDES_EN
    .filter(en => hotelGuidesFr.some(g => g.id === en.id))
    .map(g => ({ slug: g.slug }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `/fr/hotels/guides/${slug}`,
      languages: {
...
export default async function HotelGuidePageFr({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const entry = findGuide(slug);
  if (!entry) notFound();
  ...
  const entries = guide.hotelIds
    .map(id => {
      ...
      const hFr = hotelsFr.find(h => h.id === id);
      return h && hFr ? { en: h, fr: hFr } : null;
    })
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
            <thead>
              <tr style={{ borderBottom: "2px solid #e8ddd4" }}>
                <th style={{ textAlign: "left", padding: "10px 12px", color: "#1c1917" }}>Hôtel</th>
            ...
            <tbody>
              {entries.map(({ en, fr: hFr }) => (
                <tr key={en.id} style={{ borderBottom: "1px solid #e8ddd4" }}>
                  <td style={{ padding: "10px 12px", fontWeight: 600, color: "#1c1917" }}>{hFr.name}</td>
        ...
        <div key={en.id} style={{ margin: "40px 0", paddingTop: 32, borderTop: "1px solid #e8ddd4" }}>
          {en.photo && (
            <div style={{ position: "relative", width: "100%", height: 220, borderRadius: 14, overflow: "hidden", marginBottom: 20 }}>
              <Image src={`/hotels/${en.photo}`} alt={hFr.name} fill style={{ objectFit: "cover", objectPosition: "center" }} />
            </div>
          )}
          ...
          <Link href={`/fr/hotels/${en.id}`} style={{ fontSize: 14, color: "#d4711a", fontWeight: 600, textDecoration: "none", display: "inline-block", marginTop: 8 }}>
            Lire notre guide complet sur {hFr.name} →
          </Link>
        </div>
      ))}
