...
const READY_GUIDES = hotelGuides.filter(g => g.intro && g.blurbs);
...
export default async function HotelGuidePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const guide = READY_GUIDES.find(g => g.slug === slug);
  if (!guide) notFound();
    ...
    <div style={{ maxWidth: 780, margin: "0 auto", padding: "40px 24px 80px", position: "relative" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
            <thead>
              <tr style={{ borderBottom: "2px solid #e8ddd4" }}>
                <th style={{ textAlign: "left", padding: "10px 12px", color: "#1c1917" }}>Hotel</th>
            ...
            <tbody>
              {entries.map(h => (
                <tr key={h.id} style={{ borderBottom: "1px solid #e8ddd4" }}>
                  ...
                  <td style={{ padding: "10px 12px", color: "#6b6560" }}>{guide.bestFor?.[h.id]}</td>
      ...
      {entries.map(h => (
        <div key={h.id} style={{ margin: "40px 0", paddingTop: 32, borderTop: "1px solid #e8ddd4" }}>
          ...
          {guide.blurbs?.[h.id] && <ArticleBody text={guide.blurbs[h.id]} />}
          {guide.bestFor?.[h.id] && (
            <p style={{ fontSize: 14, color: "#292524", marginTop: 12 }}><strong>Best for:</strong> {guide.bestFor[h.id]}</p>
          )}
          <Link href={`/hotels/${h.id}`} style={{ fontSize: 14, color: "#d4711a", fontWeight: 600, textDecoration: "none", display: "inline-block", marginTop: 8 }}>
            Read our complete guide to {h.name} →
          </Link>
        </div>
      ))}
