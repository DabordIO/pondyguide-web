...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: buildOpenGraph({ path: `/fr/history/lives-and-legacies/${slug}`, image: entry.en.photo ? `/figures/${entry.en.photo}` : undefined, locale: "fr_FR" }),
    alternates: {
...
export default async function FigurePageFr({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "40px 24px 80px", position: "relative" }}>
      ...
      />
      <Breadcrumbs items={[{ label: "Accueil", href: "/fr" }, { label: "Histoire", href: "/fr/history" }, { label: "Vies & Héritages", href: "/fr/history/lives-and-legacies" }, { label: fr.name, href: `/fr/history/lives-and-legacies/${slug}` }]} />
      ...
      {en.photo && (
        <div style={{ display: "flex", gap: 24, margin: "24px 0 28px", alignItems: "flex-start" }}>
          <div>
            ...
            {fr.photoCaption && (
              <p style={{ fontSize: 11, color: "#a39990", fontStyle: "italic", marginTop: 6, maxWidth: 160, lineHeight: 1.4 }}>{fr.photoCaption}</p>
            )}
          </div>
        </div>
      )}
