...
function GuideCard({ title, slug, photos, span }: { title: string; slug: string; photos: string[]; span: number }) {
  ...
  return (
    <Link href={`/restaurants/guides/${slug}`} className="guide-card" style={{ gridColumn: `span ${span}`, textDecoration: "none", background: "#fff", border: "1px solid #e8ddd4", borderRadius: 14, overflow: "hidden", display: "flex", flexDirection: "column", height: "100%", minHeight: 180 }}>
...
export default function RestaurantsPage() {
  return (
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: "56px 24px 80px", position: "relative" }}>
      <LanguageToggle enHref="/restaurants" taHref="/ta/restaurants" frHref="/fr/restaurants" current="en" />
      <Breadcrumbs items={[{ label: "Home", href: "/" }, { label: "Restaurants", href: "/restaurants" }]} />
