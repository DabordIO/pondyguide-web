...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: buildOpenGraph({ path: `/discover/explore/${slug}`, image: a.photo ? `/explore/${a.photo}` : undefined }),
    alternates: {
...
export default async function ExploreArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "40px 24px 80px", position: "relative" }}>
      ...
      <JsonLd
        data={
          article.category === "walk" || article.category === "daytrip"
            ? {
                "@context": "https://schema.org",
                "@type": "TouristTrip",
                name: displayH1(article.title),
                description: article.teaser,
                image: article.photo ? `https://www.pondyguide.com/explore/${article.photo}` : undefined,
                url: `https://www.pondyguide.com/discover/explore/${article.id}`,
                touristType: "https://schema.org/Tourist",
                ...(article.duration ? { itinerary: { "@type": "ItemList", name: article.title, description: `Duration: ${article.duration}` } } : {}),
              }
            : {
                "@context": "https://schema.org",
                "@type": "Article",
                headline: displayH1(article.title),
                description: article.teaser,
                image: article.photo ? `https://www.pondyguide.com/explore/${article.photo}` : undefined,
                url: `https://www.pondyguide.com/discover/explore/${article.id}`,
              }
