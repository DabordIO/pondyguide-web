...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: buildOpenGraph({ path: `/discover/white-town/${slug}`, image: s.photo ? `/streets/${s.photo}` : undefined }),
    alternates: {
      canonical: `/discover/white-town/${slug}`,
      languages: {
    ...
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "40px 24px 80px", position: "relative" }}>
      ...
      <JsonLd
        data={{
          ...
          "@type": "Place",
          name: displayH1(street.name),
          alternateName: street.altName,
          description: street.summary,
          image: street.photo ? `https://www.pondyguide.com/streets/${street.photo}` : undefined,
          url: `https://www.pondyguide.com/discover/white-town/${street.id}`,
          geo: { "@type": "GeoCoordinates", latitude: street.lat, longitude: street.lng },
          address: { "@type": "PostalAddress", addressLocality: "White Town, Puducherry", addressCountry: "IN" },
        }}
      />
      ...
      {street.photo && (
        <div style={{ position: "relative", width: "100%", height: 300, borderRadius: 14, overflow: "hidden", margin: "24px 0 28px" }}>
          <Image src={`/streets/${street.photo}`} alt={street.name} fill priority style={{ objectFit: "cover", objectPosition: street.photoPosition ?? "center" }} />
        </div>
      )}
